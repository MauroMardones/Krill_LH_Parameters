...
Made VB curves by methot and strata

```{r}
# Definir parámetros de la curva de von Bertalanffy para diferentes grupos

# Crear una secuencia de edades para predecir la longitud
edades <- seq(0, 10, by = 0.1)

# Generar datos para cada grupo
datos <- do.call(rbind, lapply(1:nrow(parametros), function(i) {
  longitudes_pred <- parametros$L_inf[i] * (1 - exp(-parametros$K[i] * (edades - parametros$t0[i])))
  data.frame(grupo = parametros$grupo[i], edad = edades, longitud = longitudes_pred)
}))

# Tramar las curvas de von Bertalanffy utilizando ggplot2
ggplot(datos, aes(x = edad, y = longitud, color = grupo)) +
  geom_line()+
  labs(title = "Curvas de von Bertalanffy para diferentes grupos",
       x = "Edad",
       y = "Longitud") +
  theme_few()
