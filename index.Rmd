...
Some exploration in R-L relationship.
...
```{r}
lfqbin2 <- ELEFAN(
  lfq = lfq_results$SSIW,  MA = 7,
  Linf_range = seq(4.5, 7, length.out = 30),
  K_range = exp(seq(log(0.1),log(2), length.out = 30)),
  method = "cross",
...
res_RSA <- ELEFAN(lfq_results$EI , Linf_range = seq(4,6.5,1), MA = 5,
...
plot(res_RSA, Fname = "rcounts",date.axis = "modern", ylim=c(0,8))
lt <- lfqFitCurves(res_RSA, par = list(Linf=7.0, K=0.2, 
