...
In this exercises different bioanalogic methods are tested
```{r}
lfq_results$EI <- c(lfq_list, sa_res1$par, list(agemax = sa_res1$agemax))

# use the function M_empirical to estimate natural mortality
Ms <- M_empirical(Linf = lfq_list2$Linf, K_l = lfq_list2$K, 
                  tmax = lfq_list2$agemax, temp = 20.2, 
                  method = c("Pauly_Linf", "Hoenig", 
                             "Then_tmax", "Then_growth"))

# True M: 0.2
# take mean and standard deviation of the M estimates
print(paste(round(mean(as.numeric(Ms)),3), "±", round(sd(as.numeric(Ms)),3)))
# add the mean value to our data list
lfq_list2$M <- mean(as.numeric(Ms))
# alternativly use only Then_growth method:
lfq_list2$M <- Ms[5]
# M = 0.365
